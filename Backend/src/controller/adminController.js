...
async function uploadInstructorMapping(courseName, courseDesc, instructorEmail) {
    try {
...
/**
 * Handles the upsert logic for a single course-TA mapping row (No course description update).
 * If the course exists, it pushes the TA email if not present.
...
async function uploadTAMapping(courseName, taEmail) {
    try {
        ...
        const result = await CourseMapping.findOneAndUpdate(
            ...
            // 2. UPDATE: Conditionally push the new TA email
            {
                // $addToSet ensures the email is only added if it's NOT already in the array
