...
export default function InstructorPage() {
    ...
    function markAnswered(id) {
        setQuestions(s => s.map(q => q.id === id ? { ...q, status: "answered", answeredAt: new Date().toISOString() } : q));
    }
    ...
    const filtered = questions.filter(q => filter === "all" ? true : filter === "unanswered" ? q.status !== "answered" : q.status === "answered");
            ...
            <div className="w-full max-w-6xl">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <div className="text-sm text-slate-600">Lecture: <strong>{courseId}</strong> • Instructor: <strong>{instrId}</strong></div>
                    </div>

                    <div className="flex gap-2">
                        <Link to="/" className="px-3 py-2 border rounded text-slate-700">Home</Link>
                        <button onClick={archiveAndClose} className="px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Archive & Close</button>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow p-4 mb-6 flex flex-col md:flex-row md:items-center gap-3">
                    ...
                    <div className="ml-auto flex gap-2">
                        <button onClick={clearAnswered} className="px-3 py-2 border rounded text-slate-700">Clear Answered</button>
                        <button onClick={() => { navigator.clipboard?.writeText(JSON.stringify(questions)); alert("Questions JSON copied"); }} className="px-3 py-2 border rounded text-slate-700">Copy JSON</button>
                    </div>
                </div>

                <div className="space-y-3">
                    {filtered.length === 0 && <div className="text-slate-500">No questions match this filter.</div>}
                    {filtered.map(q => (
                        <div key={q.id} className="bg-white rounded-lg p-4 shadow-sm flex justify-between items-start">
                            ...
                            <div className="flex flex-col items-end gap-2">
                                ...
                                {q.status !== "answered" ? (
                                    <button onClick={() => markAnswered(q.id)} className="px-3 py-2 bg-emerald-600 text-white rounded">Mark as answered</button>
                                ) : (
